...
class PickleDataset:
    ...
    #### for NLP tasks
    def build_vocab(self, vocab_save_root: str = None, vector_save_root: str = None, vocab_limit_size: int = 50000):
        ...
        """
        save_root = Path(__file__).parent / "nlp_utils/dataset_vocab" if vocab_save_root is None else Path(vocab_save_root)
        save_root.mkdir(parents=True, exist_ok=True)
        save_file_path = save_root / f"{self.dataset_name}_vocab.pkl"
        if save_file_path.exists():
            logging.critical(f'There has been a built vocab file {self.dataset_name}_vocab.pkl for {self.dataset_name} '
                             f'dataset in {save_file_path.resolve()}, please delete it before re-building')
            raise RuntimeError("Existing vocab file {}_vocab.pkl in {}".format(self.dataset_name, save_file_path.resolve()))
        ...
        vector_save_root = Path(__file__).parent / "nlp_utils/glove" if vector_save_root is None else Path(vector_save_root)
        vocab_util = Vocab(data_tokens=all_data_token, word_dim=300, vocab_limit_size=vocab_limit_size,
                           vectors_path=vector_save_root, vector_name='glove.6B.300d.txt')
