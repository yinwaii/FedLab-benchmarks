...
class RCV1(Dataset):
    ...
    def __init__(self, root,
        ...
        self.full_file_name = {
            'train': {'data': os.path.join(root, f"rcv1_train_X_{train_ratio:.2f}.npy"),
                      'targets': os.path.join(root, f"rcv1_train_y_{train_ratio:.2f}.npy")},
            'test': {
                'data': os.path.join(root, f"rcv1_test_X_{1 - train_ratio:.2f}.npy"),
                'targets': os.path.join(root, f"rcv1_test_y_{1 - train_ratio:.2f}.npy")}}
    ...
    def generate(self):
        ...
        num_train = int(orig_X_train.shape[0] * self.train_ratio)
        orig_y_train = (orig_y_train + 1) / 2
        idxs = np.random.permutation(orig_X_train.shape[0])
